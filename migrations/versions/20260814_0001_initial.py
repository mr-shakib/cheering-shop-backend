...
The DDL below is embedded verbatim rather than read from db/schema.sql at run
time. Migrations must be immutable snapshots: if this file read an external
script, editing that script later would silently rewrite history for every
environment that had already applied revision 0001.

db/schema.sql remains the authoring source and stays byte-identical to this
block — tests/test_schema_parity.py fails the build if they drift.

Verified against PostgreSQL 16 / PostGIS 3.4: all 21 invariant assertions in
db/verify_constraints.sql pass against the resulting schema.
"""
...
def downgrade() -> None:
    op.execute("DROP TABLE IF EXISTS rider_location_pings_default CASCADE")
    op.execute("DROP TABLE IF EXISTS rider_location_pings_2026_10 CASCADE")
    op.execute("DROP TABLE IF EXISTS rider_location_pings_2026_09 CASCADE")
    op.execute("DROP TABLE IF EXISTS rider_location_pings_2026_08 CASCADE")
    op.execute("DROP TABLE IF EXISTS rider_location_pings CASCADE")
    op.execute("DROP TABLE IF EXISTS reviews CASCADE")
    op.execute("DROP TABLE IF EXISTS promo_redemptions CASCADE")
    op.execute("DROP TABLE IF EXISTS order_status_history CASCADE")
    op.execute("DROP TABLE IF EXISTS order_item_add_ons CASCADE")
    op.execute("DROP TABLE IF EXISTS order_items CASCADE")
    op.execute("DROP TABLE IF EXISTS orders CASCADE")
    op.execute("DROP TABLE IF EXISTS rider_profiles CASCADE")
    op.execute("DROP TABLE IF EXISTS promo_codes CASCADE")
    op.execute("DROP TABLE IF EXISTS cart_item_add_ons CASCADE")
    op.execute("DROP TABLE IF EXISTS cart_items CASCADE")
    op.execute("DROP TABLE IF EXISTS carts CASCADE")
    op.execute("DROP TABLE IF EXISTS item_add_ons CASCADE")
    op.execute("DROP TABLE IF EXISTS item_variants CASCADE")
    ...
    op.execute("DROP TABLE IF EXISTS biometric_credentials CASCADE")
    ...
    op.execute("DROP TYPE IF EXISTS otp_purpose CASCADE")
    ...
    op.execute("DROP TYPE IF EXISTS restaurant_status CASCADE")
    op.execute("DROP TYPE IF EXISTS address_type CASCADE")
