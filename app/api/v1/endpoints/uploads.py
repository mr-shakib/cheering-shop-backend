...
@router.post("/presigned-url", summary="Get a Cloudflare R2 presigned upload URL")
async def create_presigned_url(body: PresignedUrlRequest, user: CurrentUser):
    ...
    uploads straight to Cloudflare R2.
    ...
    header is part of the SigV4 signature, R2 itself rejects a mismatch.
    ...
    Returns 503 when R2 is not configured — nothing is broken, the feature
    simply is not provisioned in this environment. `public_url` points at the
    bucket's public domain, not at the signing endpoint, which refuses
